(function () {
  ...
  function currentSessionDraft() {
    return { sessionId: '', sessionName: attCurrentName(), startTime: $('#atStartTime').value, endTime: $('#atEndTime').value, marks: {} };
  }
  function findCurrentSavedSession() {
    var d = currentSessionDraft();
    return (atState.sessions || []).filter(function (s) { return s.sessionName === d.sessionName && s.startTime === d.startTime && s.endTime === d.endTime; })[0] || null;
  }
  function applyCurrentSession(auto) {
    var d = currentSessionDraft();
    if (!d.sessionName || !d.startTime || !d.endTime) { if (!auto) toast('請完整填寫時段名稱與時間', true); return false; }
    var found = findCurrentSavedSession();
    atState.sessionId = found ? (found.sessionId || '') : '';
    atState.sessionName = d.sessionName;
    atState.startTime = d.startTime;
    atState.endTime = d.endTime;
    atState.filter = 'all';
    return true;
  }
  ...
  async function renderAttMonth() {
    ...
    if (atState.dirty) { $('#atMonth').innerHTML = '<p class="muted">目前這個時段點名尚未儲存。請先按上方「💾 儲存點名」，再產生本月統計。</p>'; return; }
    $('#atMonth').innerHTML = '<p class="muted">統計中…</p>';
    var dateForMonth = attValidDateKey(atState.date) || attValidDateKey(($('#atDate') || {}).value) || attTodayKey();
    atState.date = dateForMonth;
    if ($('#atDate')) $('#atDate').value = dateForMonth;
    if (!atState.teamId && $('#atTeam')) atState.teamId = $('#atTeam').value || '';
    var ym = dateForMonth.slice(0, 7), from = ym + '-01', to = attMonthEnd(ym);
    ...
    if (!recs.length) {
      var team = (state.teams.filter(function (t) { return String(t.teamId) === String(atState.teamId); })[0] || {}).teamName || (($('#atTeam') && $('#atTeam').options[$('#atTeam').selectedIndex] || {}).text) || atState.teamId || '未選隊伍';
      var any = await TP.callAuth('attendanceRange', { from: from, to: to });
      var anyRecs = ((any.ok && any.records) || []).filter(function (r) { return attDateKey(r.date).slice(0, 7) === ym; });
      var hint = anyRecs.length
        ? '後端有讀到本月點名 ' + anyRecs.length + ' 個時段，但不是目前選取隊伍「' + esc(team) + '」。請切換隊伍，或檢查這批點名是否由另一個教練 / 助教帳號建立。'
        : '後端本月沒有回傳任何點名時段。請確認已按「儲存點名」，且 Apps Script 後端已更新到新版 Code.gs。';
      $('#atMonth').innerHTML =
        '<div class="empty-state" style="text-align:left;">' +
          '<b>本月尚無可統計的點名紀錄</b><br>' +
          '<span class="muted">查詢月份：' + esc(ym) + '｜範圍：' + esc(from) + ' ～ ' + esc(to) + '</span><br>' +
          '<span class="muted">目前隊伍：' + esc(team) + '｜teamId：' + esc(atState.teamId || '') + '</span><br>' +
          '<span class="muted">後端回應：' + (h.ok ? '正常，但 records=0' : esc(h.error || '失敗')) + '</span><br>' +
  ...
  $('#atSessionName').onchange = function () { $('#atCustomWrap').classList.toggle('hidden', this.value !== '自訂'); attConfirmDiscard(function () { if (applyCurrentSession(true)) loadAttDay(); }); };
  $('#atStartTime').onchange = function () { attConfirmDiscard(function () { if (applyCurrentSession(true)) loadAttDay(); }); };
  $('#atEndTime').onchange = function () { attConfirmDiscard(function () { if (applyCurrentSession(true)) loadAttDay(); }); };
  $('#atCustomSession').onchange = function () { attConfirmDiscard(function () { if (applyCurrentSession(true)) loadAttDay(); }); };
  $('#atOpenSession').onclick = function () { attConfirmDiscard(function () { if (applyCurrentSession(false)) loadAttDay(); }); };
  ...
  $('#atSave').onclick = async function () {
    if (DEMO) { demoBlock(); return; }
    if (!applyCurrentSession(false)) return;

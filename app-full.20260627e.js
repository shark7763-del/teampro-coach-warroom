(function () {
  ...
  async function renderAttMonth() {
    ...
    if (atState.dirty) { $('#atMonth').innerHTML = '<p class="muted">目前這個時段點名尚未儲存。請先按上方「💾 儲存此時段點名」，再產生本月統計。</p>'; return; }
    ...
    if (!recs.length) {
      ...
      var hint = anyRecs.length
        ...
        : '後端本月沒有回傳任何點名時段。請確認已按「儲存此時段點名」，且 Apps Script 後端已更新到新版 Code.gs。';
      $('#atMonth').innerHTML =
